  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      <button
        ...
        className="w-full flex items-center justify-between p-5 hover:bg-gray-50 dark:hover:bg-gray-700 transition text-left"
      >
        <div className="min-w-0">
          <h4 className="text-sm font-semibold text-gray-800 dark:text-gray-200">{cls.className}</h4>
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            <span className="text-xs text-gray-500 dark:text-gray-400">{cls.sectionName}</span>
            {cls.academicYear && (
              <>
                <span className="text-gray-300 dark:text-gray-600">·</span>
                <span className="text-xs text-gray-500 dark:text-gray-400">{cls.academicYear}</span>
              </>
            )}
      ...
      {/* Expanded Content */}
      {expanded && (
        <div className="border-t border-gray-100 dark:border-gray-700 p-5 bg-gray-50 dark:bg-gray-700/50">
          {loading ? (
            <div className="flex items-center justify-center py-4"><LoadingSpinner /></div>
          ) : (
            <>
              {/* Enrolled Subjects */}
              <h5 className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide mb-3">Enrolled Subjects</h5>
              {cls.enrolledSubjects?.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
                  {cls.enrolledSubjects.map(sub => (
                    <div key={sub.subjectId}
                      className="flex items-center gap-2 p-2.5 rounded-lg bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700">
                      <span className="w-2 h-2 rounded-full bg-emerald-400 flex-shrink-0" />
                      <span className="text-sm text-gray-800 dark:text-gray-200 font-medium">{sub.subjectName}</span>
              ...
              {curriculum?.subjects && curriculum.subjects.length > 0 && cls.classType === 'MODULE_BASE' && (
                <>
                  <h5 className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide mb-3">Modules</h5>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 ml-3">
                          {sub.modules.map(mod => (
                            <div key={mod.moduleId}
                              className="flex items-center gap-2 p-2 rounded-lg bg-white dark:bg-gray-800 border border-purple-100 dark:border-purple-800">
    ...
    <div className="space-y-6">
      ...
      {classes.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">You are not enrolled in any classes yet.</p>
