...
import { updateTeacher } from '../../services/managementService';
...
const TeacherProfilePage = () => {
  const { teacherProfile: ctxProfile, loading, error: ctxError } = useTeacher();
  ...
  const [editForm, setEditForm] = useState({ nic: '', teacherId: '' });
  ...
  const openEdit = () => {
    setEditForm({
      nic: resolvedProfile?.nic ?? '',
      teacherId: resolvedProfile?.teacherId ?? '',
    });
    setEditErrs({});
    setEditOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const updated = await updateTeacher(resolvedProfile.id, {
  ...
  const initials = (resolvedProfile?.username ?? 'T')
    .split(' ')
    .map((p) => p[0])
    .join('')
    .toUpperCase()
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        action={
          <button
            onClick={openEdit}
            className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-orange-600 hover:bg-orange-700 rounded-lg transition"
          >
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div className="px-6 py-4">
          ...
          <Field label="Teacher ID" value={resolvedProfile?.teacherId} />
