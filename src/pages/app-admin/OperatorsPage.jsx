...
import {
  ...
  getOperatorsByAppAdmin, createOperator, updateOperator, deleteOperator,
...
const OperatorsPage = () => {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      const data     = await getOperatorsByAppAdmin(appAdmin.id);
  ...
  // ── Edit ──────────────────────────────────────────────────────────────────
  const openEdit  = (row) => { setEditTarget(row); setEditForm({ nic: row.nic ?? '' }); setEditErrors({}); };
  const closeEdit = () => setEditTarget(null);

  const handleSave = async () => {
    setSaving(true);
    try {
      await updateOperator(editTarget.id, { nic: editForm.nic || undefined });
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Create and manage operators for your school."
        action={
          <button
            onClick={openCreate}
      ...
      <DataTable
        ...
        loading={loading}
        error={fetchErr}
        emptyMessage="No operators found. Add one to get started."
        onRetry={fetchList}
      />
